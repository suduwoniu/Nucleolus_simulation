...
%shape_num: 100 times for simulation of cut process
for shape_num=1:100
...
number_uniq=unique(number_ind);
number_count=hist(number_ind,number_uniq);
save_statics=['statics-',num2str(shape_num),'.txt'];
stat=fopen(save_statics,'wt');
for ii=0:15
    if ismember(ii,number_uniq)==1
        fprintf(stat,'%d\t%d\n',ii,number_count(number_uniq==ii));
    else
        fprintf(stat,'%d\t%d\n',ii,0);
    end
end
